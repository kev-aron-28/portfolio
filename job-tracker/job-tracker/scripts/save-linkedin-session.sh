...
OUTPUT="${1:-$HOME/linkedin-storage-state.json}"

cd "$ROOT"

if grep -qi microsoft /proc/version 2>/dev/null; then
  export DISPLAY="${DISPLAY:-:0}"
  echo "==> WSL detectado (DISPLAY=$DISPLA
  echo ""
  echo "    Si la ventana de Chromium sale en blanco, usa PowerShell en Windows:"
  echo "      cd $ROOT"
  echo "      .\\scripts\\save-linkedin-session.ps1"
  echo ""
  echo "    Luego en la app configura la ruta WSL, por ejemplo:"
  echo "      /mnt/c/Users/TU_USUARIO/linkedin-storage-state.json"
  echo ""
fi
...
./mvnw exec:java@linkedin-session -Dsession.file="$OUTPUT"
