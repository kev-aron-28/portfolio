...
OUTPUT="${1:-$HOME/indeed-storage-state.json}"
...
if grep -qi microsoft /proc/version 2>/dev/null; then
  ...
  echo "      cd $ROOT"
  echo "      .\\scripts\\save-indeed-session.cmd"
  echo ""
  echo "    Luego en la app configura la ruta WSL, por ejemplo:"
  echo "      /mnt/c/Users/TU_USUARIO/indeed-storage-state.json"
  echo ""
fi
...
./mvnw exec:java@indeed-session -Dindeed.session.file="$OUTPUT"
